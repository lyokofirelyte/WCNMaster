...
public class WebsiteManager implements Runnable, AutoRegister<WebsiteManager> {
	...
	@Getter
	private WebsiteManager type = this;
	
	public WebsiteManager(Empyreal i){
		api = i;
	}
					...
					if (!messages.contains(s)){
						...
						for (String m : send.split(" ")){
							...
							if (m.startsWith("http")){
								extra = new JSONChatExtra(api.AS("&6&o" + api.getInstance(TitleExtractor.class).getType().getPageTitle(m) + " "));

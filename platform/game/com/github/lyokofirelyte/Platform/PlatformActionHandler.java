...
public class PlatformActionHandler implements AutoRegister<PlatformActionHandler> {

	public Platform main;
	
	@Getter
	private PlatformActionHandler type = this;
	
	public PlatformActionHandler(Platform i){
		main = i;
	}
	...
	public void formArena(Location center){
		...
		for (int x = 0; x < 2; x++){
			for (Location l : Utils.circle(center, 23+x, 1, true, false, 2)){
	...
	public void phaseTwo(List<Location> arenaLocs){
		...
		Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(main, new Runnable(){
			...
			public void run(){
				...
				Bukkit.getPluginManager().callEvent(new RoundEndEvent(null, Round.ONE));
			}

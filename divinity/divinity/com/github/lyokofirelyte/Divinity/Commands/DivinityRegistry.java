...
public class DivinityRegistry implements CommandExecutor {
	...
	public DivinityRegistry(API api, Divinity div){
		main = api;
		divinity = div;
	}
	...
	public static void args(Object clazz, CommandSender s, String cmd, String[] args){
		...
		try {
							...
							if (s.hasPermission(anno.perm())){
								...
								if (anno.player()){
									if (s instanceof Player){
										...
										DivinityUtilsModule.s(s, "&cConsole players cannot run this command!");
									}
								} else {
									m.invoke(clazz, s, args);
								}
							} else {
								DivinityUtilsModule.s(s, "&cYou don't have permission for that!");
							}
						}
					}
				}
			...
			if (!(triggered)){
				DivinityUtilsModule.s(s, "&cThere's a time and a place for everything!");
			}
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    	...
    	if (filteredCmds.contains(cmd)){
	    	DivinitySystem system = main.getDivSystem();
    						...
    						if (m.getAnnotation(DivCommand.class) != null && Arrays.asList(m.getAnnotation(DivCommand.class).aliases()).contains(command)){
    							try {
    								...
    								if ((sender instanceof Player && main.getDivPlayer((Player)sender).getList(DPI.PERMS).contains(anno.perm())) || sender instanceof Player == false || sender.isOp()){
    									...
    									if (anno.name().equals("none")){
    										if (anno.player()){
    											if (sender instanceof Player){
    												...
    												DivinityUtilsModule.s(sender, "&cConsole players cannot run this command!");
    											}
    										} else {
    									...
    									DivinityUtilsModule.s(sender, "&4No permission!");
    								}
    							} catch (Exception e) {
    								e.printStackTrace();
    							}
